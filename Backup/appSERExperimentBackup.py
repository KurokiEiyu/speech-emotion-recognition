...
model = load_model('dataset/model_hybrid.h5')
...
def extract_hybrid_features(filename):
    try:
        y, sr = librosa.load(filename, sr=fs, duration=3.0, offset=0.5)
        if len(y) < 10000:
...
def index():
    history = session.get('history', [])
    return render_template('indexExperimentBackup.html', history=history)
...
def result(emotion, accuracy):
    ...
    history = session.get('history', [])
    return render_template('indexExperimentBackup.html', emotion=emotion, accuracy=round(accuracy * 100, 2), error=error, audio_last_path=audio_last_path, history=history)
